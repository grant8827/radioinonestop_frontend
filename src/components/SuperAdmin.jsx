  ...
  return <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
    ...
    <div className="min-h-80 rounded-xl border border-gray-800 bg-gray-900 p-6">
      {!selected ? <div className="flex h-full items-center justify-center text-sm text-gray-500">Select a message to read it.</div> : <div>
        <div className="border-b border-gray-800 pb-4"><p className="text-xs uppercase tracking-wide text-purple-400">{labels[selected.reason] || selected.reason}</p><h2 className="mt-1 font-semibold text-white">{selected.email}</h2>{selected.station && <p className="mt-2 break-all text-sm text-cyan-400"><span className="text-gray-500">Station: </span>{selected.station}</p>}<p className="mt-1 text-xs text-gray-500">{new Date(selected.createdAt).toLocaleString()}</p></div>
