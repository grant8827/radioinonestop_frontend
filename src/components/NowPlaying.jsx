...
export default function NowPlaying({ config, mode }) {
  ...
  const radioLive = radioStatus === 'live'
  const radioConnecting = radioStatus === 'connecting'
  ...
  return (
    <div className="bg-gray-900 rounded-xl px-4 py-3 flex items-center gap-3 flex-wrap">
      ...
      <button
        onClick={radioLive ? stopRadio : startRadio}
      ...
      >
        ...
        {radioLive ? '● GO LIVE RADIO' : radioConnecting ? 'CONNECTING…' : radioStatus === 'error' ? 'RETRY LIVE' : 'GO LIVE RADIO'}
      </button>
    </div>
  )
}
