...
export default function NowPlaying({ config, mode }) {
  ...
  const radioConnecting = liveStatus === 'connecting' || liveStatus === 'requesting'
  ...
  return (
    <div className="bg-gray-900 rounded-xl px-4 py-3 flex items-center gap-3 flex-wrap">
      ...
      >
        ...
        {radioLive ? '● GO LIVE RADIO' : radioConnecting ? 'CONNECTING…' : liveStatus === 'error' ? 'RETRY LIVE' : 'GO LIVE RADIO'}
      </button>
    </div>
  )
}
