      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-gray-950 p-6 shadow-2xl">
        ...
        {status === 'sent' ? (
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <div><label className="mb-2 block text-sm font-medium text-gray-200">Station name or station URL {reason === 'station_not_streaming' ? <span className="text-red-400">*</span> : <span className="text-gray-500">(optional)</span>}</label><input type="text" required={reason === 'station_not_streaming'} maxLength={300} value={station} onChange={(e) => setStation(e.target.value)} placeholder="My Station or radioinonestop.com/?station=..." className="w-full rounded-lg border border-gray-800 bg-gray-900 px-3 py-2.5 text-sm text-white outline-none focus:border-red-500" /><p className="mt-1.5 text-xs text-gray-500">This helps us find the correct station and stream.</p></div>
